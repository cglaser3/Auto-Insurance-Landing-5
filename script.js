  ...
  async function fetchMakesForYear() {
    if (!makeCache) {
      ...
      try {
        ...
        makeCache = data.Results.map((r) => ({ id: r.MakeId, name: r.MakeName }))
    ...
    if (!modelCache[key]) {
      const url =
        `https://vpic.nhtsa.dot.gov/api/vehicles/GetModelsForMakeYear/make/${encodeURIComponent(
          make
        )}/modelyear/${year}/vehicletype/passenger%20car?format=json`;
      try {
        const res = await fetch(url);
        const data = await res.json();
        modelCache[key] = data.Results.map((r) => r.Model_Name).sort();
      } catch (err) {
  ...
  function renderStep3() {
    app.innerHTML = `
      <div class="container">
        ...
        <p>Thank you! We have recorded your vehicle information.</p>
      </div>
    `;
